        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, index) => (
              ...
              >
              {/* Image Container */}
              <div className="relative w-full h-[220px] lg:group-hover:h-[120px] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] overflow-hidden shrink-0">
                <img 
                  src={project.image} 
                  alt={project.title} 
              ...
              <div className="p-6 flex flex-col flex-grow relative bg-slate-200/80 dark:bg-[#1c1917]/80 backdrop-blur-md z-10 transition-all duration-500">
                ...
                <div className="grid grid-rows-[1fr] lg:grid-rows-[0fr] lg:group-hover:grid-rows-[1fr] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]">
                  <div className="overflow-hidden">
                    <p className="text-slate-600 dark:text-[#a8a29e] text-sm line-clamp-4 mt-4 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-500 delay-100">
                      {project.description}
                    </p>
                  </div>
                </div>
                ...
                {/* View Indicator */}
                <div className="mt-auto pt-4 flex items-center gap-2 font-mono text-xs text-slate-900 dark:text-[#f3f4f6] opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-all duration-300">
              ...
              {/* Hover Overlay */}
              <div className={`absolute inset-0 border-2 opacity-0 lg:group-hover:opacity-100 rounded-xl pointer-events-none transition-opacity duration-500 ${getCategoryStyles(project.category).cardBorder}`} />
            </motion.div>
            ))}
          </AnimatePresence>
        </motion.div>
